...
while IFS= read -r line; do
  [[ -n "$line" ]] || continue
  printf '%s' "$line" | python3 "$ROOT_DIR/tools/json_schema_validate.py" "$ROOT_DIR/docs/schemas/export_jsonl_row.json"
done < <(bash "$LM" export --jsonl)

echo "export jsonl schema ok"

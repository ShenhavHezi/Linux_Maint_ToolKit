...
for support_lib in linux_maint_runtime.sh linux_maint_admin.sh linux_maint_help.sh linux_maint_tui.sh linux_maint_advanced.sh linux_maint_history.sh linux_maint_ops.sh; do
...
old = 'METRICS_STATUS_RC="$status_rc" python3 - "$tmp_status" "$tmp_trend" "$tmp_runtimes" <<\'PY\''
new = 'printf \'%s\\n\' \'{not-json\' >"$tmp_status"\n    METRICS_STATUS_RC="0" python3 - "$tmp_status" "$tmp_trend" "$tmp_runtimes" <<\'PY\''
text = path.read_text(encoding="utf-8")
if old not in text:
    raise SystemExit("metrics test patch target not found")

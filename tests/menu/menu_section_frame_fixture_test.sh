...
EXPECTED="$ROOT_DIR/tests/fixtures/menu_section_frame.txt"

if ! command -v gum >/dev/null 2>&1; then
  echo "menu section frame fixture skipped: gum not found"
  exit 0
fi
...
if ! diff -u "$EXPECTED" "$norm" >/dev/null; then
  echo "menu section frame fixture mismatch" >&2
  diff -u "$EXPECTED" "$norm" >&2 || true
  exit 1
fi

echo "menu section frame fixture ok"

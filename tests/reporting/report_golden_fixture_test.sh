...
run_id=run-golden-report-001
logfile=/var/log/health/full_health_monitor_latest.log
S

# Wrapper logs for runtimes
cat > "$repo_logs/full_health_monitor_2026-02-24_000000.log" <<'S'
RUNTIME monitor=service_monitor ms=1200
RUNTIME monitor=network_monitor ms=2500
S

ln -sfn "full_health_monitor_2026-02-24_000000.log" "$repo_logs/full_health_monitor_latest.log"
...
out="$(NO_COLOR=1 LOG_DIR="$repo_logs" LM_CFG_DIR="$cfg_dir" bash "$LM" report --short --no-trend --no-slow --no-reasons)"

expected_file="$ROOT_DIR/tests/fixtures/report_golden.txt"
if ! diff -u "$expected_file" <(printf '%s
' "$out") >/dev/null; then
  echo "report golden output mismatch" >&2
  diff -u "$expected_file" <(printf '%s
' "$out") >&2 || true
  exit 1
fi

echo "report golden fixture ok"

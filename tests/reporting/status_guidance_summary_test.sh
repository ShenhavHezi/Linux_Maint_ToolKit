...
run_id=run-status-guidance-001
EOF

out="$(LOG_DIR="$log_dir" SUMMARY_DIR="$log_dir" LM_CFG_DIR="$cfg_dir" NO_COLOR=1 bash "$LM" status --table --problems 5 --reasons 5 2>&1 || true)"

printf '%s\n' "$out" | grep -q '^== Guidance ==$' || {
  echo "status guidance header missing" >&2
  echo "$out" >&2
  exit 1
}
printf '%s\n' "$out" | grep -q '^== Summary ==$' || {
  echo "status summary header missing" >&2
  echo "$out" >&2
  exit 1
}
printf '%s\n' "$out" | grep -q '^result=CRIT$' || {
  echo "status result summary missing" >&2
  echo "$out" >&2
  exit 1
}
printf '%s\n' "$out" | grep -q '^status crit$' || {
  echo "status final label missing" >&2
  echo "$out" >&2
  exit 1
}

echo "status guidance summary ok"

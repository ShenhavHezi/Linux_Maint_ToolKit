...
trap 'chmod 0644 "$log_dir/last_status_full" 2>/dev/null || true; rm -rf "$workdir"' EXIT
...
if [[ "$(id -u)" -eq 0 ]]; then
  if command -v su >/dev/null 2>&1 && id nobody >/dev/null 2>&1; then
    ...
    echo "status unreadable last_status skipped under root: no su/nobody"
    exit 0
  fi
else
  chmod 000 "$log_dir/last_status_full"
...
printf '%s' "$json_out" | python3 -c 'import json,sys; obj=json.load(sys.stdin); assert obj["status_json_contract_version"]==1; assert obj["last_status_file"]; assert obj["last_status_state"]=="unreadable"; assert obj["last_status_errors"]==["unreadable"]; assert obj["last_status"]=={}; assert obj["totals"]["WARN"]==1'

printf '%s\n' "$human_out" | grep -q '^Unreadable status file: ' || {
  echo "status should report unreadable status file" >&2
  echo "$human_out" >&2
  exit 1
}
...
echo "status unreadable last_status ok"

...
json_out="$(LOG_DIR="$log_dir" LM_CFG_DIR="$cfg_dir" bash "$LM" report --json)"
printf '%s' "$json_out" | python3 -c 'import json,sys; o=json.load(sys.stdin); warns=o["trend"].get("history_warnings", []); assert len(warns)==1; assert warns[0]["state"]=="malformed"; assert warns[0]["file"].endswith("000001.log")'

human_out="$(NO_COLOR=1 LOG_DIR="$log_dir" LM_CFG_DIR="$cfg_dir" bash "$LM" report)"
printf '%s\n' "$human_out" | grep -q '^history_warnings:$' || {
  echo "report should surface trend history warnings" >&2
  echo "$human_out" >&2
  exit 1
}
printf '%s\n' "$human_out" | grep -q 'skipped malformed history file: .*000001.log' || {
  echo "report missing trend history warning" >&2
  echo "$human_out" >&2
  exit 1
}

echo "report history warnings ok"

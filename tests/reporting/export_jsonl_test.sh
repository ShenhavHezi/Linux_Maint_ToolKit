...
jsonl_out="$(bash "$LM" export --jsonl)"
printf '%s\n' "$jsonl_out" | python3 -c '
import json,sys
lines=[ln for ln in sys.stdin.read().splitlines() if ln.strip()]
assert len(lines)==2
rows=[json.loads(ln) for ln in lines]
assert rows[0].get("monitor") in ("health_monitor","network_monitor")
assert rows[1].get("monitor") in ("health_monitor","network_monitor")
assert any(r.get("reason")=="ping_failed" for r in rows)
print("export jsonl ok")
'

redact_out="$(LM_REDACT_LOGS=1 bash "$LM" export --jsonl)"
printf '%s\n' "$redact_out" | python3 -c '
import json,sys
for ln in sys.stdin:
    ln=ln.strip()
    if not ln:
...
print("export jsonl redaction ok")
'

allow_out="$(LM_EXPORT_ALLOWLIST=monitor,host,status bash "$LM" export --jsonl)"
printf '%s\n' "$allow_out" | python3 -c '
import json,sys
for ln in sys.stdin:
    ln=ln.strip()
...
print("export jsonl allowlist ok")
'

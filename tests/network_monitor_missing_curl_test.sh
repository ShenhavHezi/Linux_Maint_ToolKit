...
TARGETS=${TMPDIR}/network_targets_test.csv
cat > "$TARGETS" <<'CSV'
...
out=$(TARGETS="$TARGETS" LM_LOGFILE=${TMPDIR}/network_monitor_missing_curl.log bash "$ROOT_DIR/monitors/network_monitor.sh" 2>&1 || true)

...
new = 'st_json="$(printf \'%s\\n\' \'{\"last_status\":{\"overall\":\"OK\"},\"reason_rollup\":[]}\')"\n    st_rc=0'
...
if [[ "$rc" -ne 2 ]]; then
  echo "expected ai-assist missing contract version failure rc=2, got rc=$rc" >&2
  echo "$out" >&2
  exit 1
fi

printf '%s\n' "$out" | grep -q '^ERROR: ai-assist requires status --json contract fields$' || {
  echo "unexpected ai-assist missing contract version output" >&2
  echo "$out" >&2
  exit 1
}

echo "ai-assist missing contract version ok"

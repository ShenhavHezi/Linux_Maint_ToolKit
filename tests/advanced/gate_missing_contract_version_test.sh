...
new = 'status_json="$(printf \'%s\\n\' \'{\"last_status\":{\"overall\":\"OK\"},\"totals\":{\"CRIT\":0,\"WARN\":0,\"UNKNOWN\":0,\"SKIP\":0,\"OK\":1}}\')"\n    gate_status_rc=0'
text = path.read_text(encoding="utf-8")
if old not in text:
    raise SystemExit("gate test patch target not found")
...
if [[ "$rc" -ne 2 ]]; then
  echo "expected gate missing contract version failure rc=2, got rc=$rc" >&2
  echo "$out" >&2
  exit 1
fi

printf '%s\n' "$out" | grep -q '^ERROR: gate requires status --json contract fields$' || {
  echo "unexpected gate missing contract version output" >&2
  echo "$out" >&2
  exit 1
}

echo "gate missing contract version ok"

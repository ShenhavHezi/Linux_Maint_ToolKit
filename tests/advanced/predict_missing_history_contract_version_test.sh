...
if [[ "$rc" -ne 2 ]]; then
  echo "expected predict missing contract version failure rc=2, got rc=$rc" >&2
  echo "$out" >&2
  exit 1
fi

printf '%s\n' "$out" | grep -q '^ERROR: predict requires history --json contract fields$' || {
  echo "unexpected predict missing contract version output" >&2
  echo "$out" >&2
  exit 1
}

echo "predict missing contract version ok"

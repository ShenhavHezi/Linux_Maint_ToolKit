...
new = 'st_json="$(printf \'%s\\n\' \'{\"status_json_contract_version\":1,\"last_status\":[],\"reason_rollup\":{}}\')"\n    st_rc=0'
text = path.read_text(encoding="utf-8")
if old not in text:
    raise SystemExit("ai-assist shape test patch target not found")
...
if [[ "$rc" -ne 2 ]]; then
  echo "expected ai-assist invalid status shape failure rc=2, got rc=$rc" >&2
  echo "$out" >&2
  exit 1
fi

printf '%s\n' "$out" | grep -q '^ERROR: ai-assist requires status --json contract fields$' || {
  echo "unexpected ai-assist invalid status shape output" >&2
  echo "$out" >&2
  exit 1
}

echo "ai-assist invalid status shape ok"

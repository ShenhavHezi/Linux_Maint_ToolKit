...
if ! command -v flock >/dev/null 2>&1; then
  echo "flock not available; skipping run lock invalid meta test"
  exit 0
fi
...
pid=not-a-pid
run_id=bad-meta
started=definitely-not-a-timestamp
EOF
...
if [[ "$rc" -ne 2 ]]; then
  echo "expected rc=2 when lock held with invalid meta, got rc=$rc" >&2
  echo "$out" >&2
  exit 1
fi

printf '%s\n' "$out" | grep -q 'owner_pid=unknown started=unknown' || {
  echo "expected sanitized lock metadata in contention output" >&2
  echo "$out" >&2
  exit 1
}

printf '%s\n' "$out" | grep -q 'not-a-pid' && {
  echo "invalid lock owner leaked into contention output" >&2
  echo "$out" >&2
  exit 1
}

echo "run lock invalid meta ok"

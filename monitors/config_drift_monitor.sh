...
run_for_host(){
  ...
  if [ "$BASELINE_ONLY" = "1" ]; then
    if [ "$had_base" -eq 1 ] && [ "$BASELINE_UPDATE" != "true" ]; then
      ...
      lm_summary "config_drift_monitor" "$host" "SKIP" reason=baseline_exists modified=0 added=0 removed=0
      rm -f "$cur_file"
      lm_info "===== Completed $host ====="
      return 0
    fi
    ...
    if [ "$had_base" -eq 1 ]; then
      ...
      lm_summary "config_drift_monitor" "$host" "OK" reason=baseline_updated modified=0 added=0 removed=0
    else
      lm_info "[$host] Baseline created at $base_file."
      lm_summary "config_drift_monitor" "$host" "OK" reason=baseline_created modified=0 added=0 removed=0
    fi
    rm -f "$cur_file"
    lm_info "===== Completed $host ====="
    return 0

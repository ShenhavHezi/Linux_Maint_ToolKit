...
LM_LOGFILE="${LM_LOGFILE:-/var/log/kernel_events_monitor.log}"
: "${LM_MAX_PARALLEL:=0}"
...
lm_require_cmd "kernel_events_monitor" "localhost" awk || exit $?
lm_require_cmd "kernel_events_monitor" "localhost" sed || exit $?
lm_require_cmd "kernel_events_monitor" "localhost" grep || exit $?
...
run_for_host(){
  ...
  if ! lm_reachable "$host"; then
    ...
    lm_summary "kernel_events_monitor" "$host" "CRIT" reason=ssh_unreachable matches=?
    # legacy:
  ...
  if [ -z "$out" ]; then
    ...
    lm_summary "kernel_events_monitor" "$host" "UNKNOWN" reason=kernel_log_unreadable matches=?
    # legacy:
